...
public class OtherMenu implements GuiBuilder {
    @Override
    public String name() {
        return "§cSanctions §7(Autre)";
    }
    ...
    public void contents(Player player, Inventory inv) {
        ...
        inv.setItem(36, new ItemBuilder(Material.ANVIL).setName("§6Catégorie: §fAutre").setLore("§r ", "§fVous êtes déjà dans cette catégorie !").toItemStack());
    ...
    public void onClick(Player player, Inventory inv, ItemStack current, int slot) throws IOException, NoSuchMethodException {
        if(inv.getName().equals("§cSanctions §7(Autre)")){
            switch (slot){
                case 18:
