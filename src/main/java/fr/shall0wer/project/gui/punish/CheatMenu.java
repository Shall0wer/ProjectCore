...
public class CheatMenu implements GuiBuilder {
    @Override
    public String name() {
        return "§cSanctions §7(Triche)";
    }
    ...
    public void contents(Player player, Inventory inv) {
        ...
        inv.setItem(27, new ItemBuilder(Material.GOLDEN_APPLE).setName("§6Catégorie: §fTriche").setLore("§r ", "§fVous êtes déjà dans cette catégorie !").toItemStack());
    ...
    public void onClick(Player player, Inventory inv, ItemStack current, int slot) throws IOException, NoSuchMethodException {
        if(inv.getName().equals("§cSanctions §7(Triche)")) {
            switch (slot) {
                case 18:
